...
describe('Name', () => {
    ...
    it('should throw an error if first name is empty', () => {
        expect(() => new Name('', 'Doe')).toThrowError('First name cannot be empty.');
    });
    it('should throw an error if last name is empty', () => {
        expect(() => new Name('John', '')).toThrowError('Last name cannot be empty.');
    });
    it('should throw an error if first name is invalid', () => {
        expect(() => new Name('John@', 'Doe')).toThrowError('Invalid name: must be 35 characters or less and contain only letters, numbers, and apostrophes.');
    });
    ...
    it('should throw an error if first name is longer than 35 characters', () => {
        ...
        expect(() => new Name(longName, 'Doe')).toThrowError('Invalid name: must be 35 characters or less and contain only letters, numbers, and apostrophes.');
    });
    it('should throw an error if last name is longer than 35 characters', () => {
        const longName = 'a'.repeat(36);
        expect(() => new Name('John', longName)).toThrowError('Invalid name: must be 35 characters or less and contain only letters, numbers, and apostrophes.');
    });
});
